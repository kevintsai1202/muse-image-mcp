...
三個工具，涵蓋在對話中處理圖片的完整流程：

| 工具 | 用途 |
|---|---|
| `generate_image` | 文字生圖。單次 1–10 張。 |
| `edit_image` | 依參考圖改圖，支援本機檔案與網址。 |
| `iterate_image` | 對話式迭代。一直說「再暖一點」，它記得前面幾輪。 |

## 為什麼用這個 server

**你的 context 不會被圖片吃掉。** 每個工具都把圖片寫進磁碟、只回傳**絕對路徑**，絕不回傳圖片內容。生成十幾張圖只花掉十幾行 context，而不是十幾 MB 的 base64。真的要看圖時，用檔案讀取工具開啟那個路徑即可。

**多輪迭代，但不存狀態。** `iterate_image` 會回傳 `response_id`；把它當作下一輪的 `previous_response_id`，就能延續同一段對話。server 自己不保存任何狀態——對話由 Meta 端保存，所以這個 server 隨時可以重啟。

**出了新模型不必等改版。** 用 `MUSE_MODEL` 或單次的 `model` 參數換模型，用 `extra_params` 送這個 server 從沒聽過的新參數。決定請求結構的欄位受保護不會被覆寫，其餘都是刻意留的逃生口。

**你隨時知道花了多少錢。** 每次回應結尾都會揭露該次的預估成本。

## 需求

- Node.js >= 20.12.0
- 一組 Meta Muse API key

### 如何取得 API key

Muse Image 跑在 **Meta Model API** 上，所以你是向 Meta 註冊，不是向本專案註冊：

1. 前往 **<https://dev.meta.ai>**，登入 Meta Model API 後台。
2. 開啟 **API keys**。
3. 點 **Create API key**，複製產生的金鑰。

這組值就是下面要填的 `MUSE_API_KEY`。Meta 官方文件把這個變數叫 `MODEL_API_KEY`；本 server 讀的名稱是 `MUSE_API_KEY`，連線到 `https://api.meta.ai/v1`，預設模型為 `muse-image-1.0`。

參考：[Model API 文件](https://dev.meta.ai/docs) · [影像生成](https://dev.meta.ai/docs/image-generation) · [Muse Image 發布說明](https://developer.meta.com/ai/resources/blog/build-with-muse-Image/)

金鑰不要進版控——用 MCP 設定的 `env` 區塊或 `.env` 檔，兩種方式都在下面說明。
...
- `--scope user` 對所有專案生效；只想裝在當前專案改用 `--scope local`。
- `-y` 跳過 npx 的安裝確認。**漏掉會卡在互動詢問而握手失敗。**
- `--` 之後才是真正的啟動指令，前面的 `--env` 屬於 `claude mcp add`。

其他 MCP client（Claude Desktop、Cursor）請手動寫設定檔：
...
Windows 若出現 `npx` 找不到，把 `command` 換成 `cmd`、`args` 改為 `["/c", "npx", "-y", "muse-image-mcp"]`。
...
這個 server 本身免費且採用 MIT 授權——你只需支付 Meta 的 API 使用費。
